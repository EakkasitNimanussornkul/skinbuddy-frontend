...
<template>
  <Teleport to="body">
    <div
      v-if="isOpen"
      class="fixed inset-0 z-50 flex flex-col justify-end sm:justify-center sm:items-center bg-slate-900/80 backdrop-blur-sm p-0 sm:p-4 animate-fade-in"
      @click.self="emit('close')"
    >
      <div class="bg-brand-surface-light dark:bg-brand-surface-dark w-full sm:max-w-md rounded-t-[2rem] sm:rounded-3xl p-6 shadow-2xl flex flex-col max-h-[90vh] animate-slide-up relative border border-slate-200 dark:border-slate-800">

        <!-- Close Button -->
        <button @click="emit('close')" class="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 bg-slate-100 dark:bg-slate-800 rounded-full transition-colors cursor-pointer">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mb-5">Type the 4-letter code or search by keyword.</p>
        ...
        <div class="relative z-20">
          <div class="relative">
            ...
            <input
              ...
              class="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-brand-text dark:text-white pl-11 pr-4 py-4 rounded-2xl font-bold text-lg focus:ring-0 focus:border-brand-primary outline-none transition-colors uppercase placeholder:text-slate-400 dark:placeholder:text-slate-600"
            />
          </div>
        ...
        <div class="flex-1 overflow-y-auto hide-scrollbar mt-4">
          ...
          />
          <div v-else class="h-32 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600">
        ...
        <div class="pt-4 border-t border-slate-100 dark:border-slate-800 mt-auto">
          <button
            ...
            :class="(!selectedType || isSaving) ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed' : 'bg-brand-primary hover:bg-brand-primary/80 text-white shadow-md active:scale-[0.98]'"
