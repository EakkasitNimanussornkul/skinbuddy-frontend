...
<template>
  <Teleport to="body">
    ...
    >
      <div class="bg-brand-surface-light dark:bg-brand-surface-dark w-full sm:max-w-md rounded-t-[2rem] sm:rounded-3xl p-6 shadow-2xl flex flex-col max-h-[90vh] animate-slide-up relative border border-stone-200 dark:border-stone-800">

        <!-- Close Button -->
        <button @click="emit('close')" class="absolute top-4 right-4 p-2 text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 bg-stone-100 dark:bg-stone-800 rounded-full transition-colors cursor-pointer">
        ...
        <div class="relative z-20">
          <div class="relative">
            ...
            <input
              ...
              class="w-full bg-stone-50 dark:bg-stone-900 border-2 border-stone-200 dark:border-stone-800 text-brand-text dark:text-white pl-11 pr-4 py-4 rounded-2xl font-bold text-lg focus:ring-0 focus:border-brand-primary outline-none transition-colors uppercase placeholder:text-stone-400 dark:placeholder:text-stone-600"
            />
          </div>
          ...
          <div
            ...
            class="absolute left-0 right-0 mt-2 bg-brand-surface-light dark:bg-brand-surface-dark border border-stone-200 dark:border-stone-700 rounded-2xl shadow-xl z-30 px-4 py-3.5 text-xs font-bold text-stone-500 dark:text-stone-400"
          >
            No matching skin types found.
          </div>

          <!-- Dropdown List -->
          <ul v-if="isDropdownOpen && filteredTypes.length > 0" class="absolute left-0 right-0 mt-2 bg-brand-surface-light dark:bg-brand-surface-dark border border-stone-200 dark:border-stone-700 rounded-2xl shadow-xl max-h-48 overflow-y-auto hide-scrollbar z-30">
            <li
              v-for="type in filteredTypes"
              :key="type"
              @click="selectType(type)"
              class="px-4 py-3 hover:bg-brand-primary/10 cursor-pointer border-b border-stone-100 dark:border-stone-700/50 last:border-0 flex justify-between items-center transition-colors"
            >
        ...
        <div class="pt-4 border-t border-stone-100 dark:border-stone-800 mt-auto">
          <button
            ...
            :class="(!selectedType || isSaving) ? 'bg-stone-200 dark:bg-stone-800 text-stone-400 cursor-not-allowed' : 'bg-brand-primary hover:bg-brand-primary/80 text-white shadow-md active:scale-[0.98]'"
            class="w-full py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
