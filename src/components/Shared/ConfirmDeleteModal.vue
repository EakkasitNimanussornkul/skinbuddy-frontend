  ...
  <div class="fixed inset-0 z-[80] flex items-center justify-center bg-stone-900/60 backdrop-blur-sm p-4 animate-fade-in" @click.self="emit('cancel')">
    <div class="bg-white dark:bg-stone-900 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-red-100 dark:border-red-900/30 animate-slide-up">
      ...
      <div class="bg-red-50 dark:bg-red-900/10 p-6 flex flex-col items-center text-center border-b border-red-100 dark:border-red-900/30">
        ...
        <p class="text-xs font-semibold text-red-600 dark:text-red-400 mt-1">This will remove it from your shelf and routine.</p>
      </div>

      <div class="p-6 bg-stone-50 dark:bg-stone-800/30 flex items-center gap-4">
        <div class="w-12 h-12 bg-white dark:bg-stone-800 rounded-xl p-1 border border-stone-200 dark:border-stone-700 flex items-center justify-center flex-shrink-0">
          <img v-if="imageUrl" :src="imageUrl" class="w-full h-full object-contain mix-blend-multiply dark:mix-blend-normal p-0.5" />
          <svg v-else class="w-6 h-6 text-stone-300 dark:text-stone-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
        </div>
        <div>
          <p class="text-[10px] font-bold text-brand-primary dark:text-stone-400 uppercase tracking-wider line-clamp-1">{{ brand }}</p>
          <p class="text-sm font-bold text-brand-text dark:text-stone-200 line-clamp-2 leading-tight mt-0.5">{{ name }}</p>
        </div>
      </div>

      <div class="p-4 grid grid-cols-2 gap-3 bg-white dark:bg-stone-900 border-t border-stone-100 dark:border-stone-800">
        <button @click="emit('cancel')" class="py-3 rounded-xl font-bold text-stone-600 dark:text-stone-300 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors text-sm">Cancel</button>
        <button @click="emit('confirm')" class="py-3 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 transition-colors shadow-md shadow-red-500/20 text-sm">Yes, Remove</button>
      </div>
