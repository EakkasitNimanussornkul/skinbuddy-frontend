  ...
  <div class="space-y-2">
    ...
    <div v-if="item.opened_date" class="bg-brand-primary-light/30 dark:bg-brand-primary/10 border border-brand-primary/20 p-4 rounded-2xl transition-all">
      ...
      <div v-else class="space-y-4 pt-1 animate-fade-in">
        <div class="flex justify-between items-center relative z-10">
          ...
          <button @click="isEditingExpiration = false" class="text-xs font-bold text-brand-text-muted hover:text-brand-primary transition-colors">Cancel</button>
        </div>

        <div class="flex overflow-x-auto gap-2 pb-2 hide-scrollbar -mx-1 px-1">
          <button
        ...
        <button @click="handleUpdateExpiration" class="w-full py-3 bg-brand-primary hover:bg-brand-primary-hover text-white dark:text-brand-text rounded-xl font-bold text-sm transition-all active:scale-[0.98] shadow-sm">Save Date</button>
      </div>
    </div>
    ...
    <div v-else class="bg-brand-surface-light dark:bg-brand-surface-dark border border-brand-surface-border dark:border-stone-800 p-4 rounded-2xl flex flex-col gap-4 shadow-sm">
      <div class="flex justify-between items-center">
        ...
        <span v-if="item.pao" class="text-xs font-bold bg-brand-bg-light dark:bg-stone-800 text-brand-text-muted px-3 py-1 rounded-lg border border-brand-surface-border dark:border-stone-700">{{ item.pao }}M PAO</span>
      </div>
      <button v-if="item.usage_state !== 'archived'" @click="handleStartPAO" class="w-full py-3.5 bg-brand-primary hover:bg-brand-primary-hover text-white dark:text-brand-text rounded-xl font-bold text-sm transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm">
        Start Product Life
      </button>
    </div>
  </div>
