...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-clinical-bg pt-6 pb-28 font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto px-4 sm:px-6">
      ...
      <div
        class="bg-white dark:bg-clinical-surface rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 mb-4 flex items-center gap-5">
        <div
          class="w-16 h-16 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-700 bg-slate-100 flex-shrink-0 flex items-center justify-center">
        ...
        <div class="flex-grow">
          ...
          <div v-if="authStore.user?.skin_type"
          ...
          <button v-else @click="router.push('/quiz')"
      ...
      <h3 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3 px-2">App
        Preferences</h3>
      <div
        class="bg-white dark:bg-clinical-surface rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-8 overflow-hidden">
        ...
        <div class="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <button @click="themeStore.toggleTheme"
            ...
            <div class="absolute top-1 bg-white w-4 h-4 rounded-full transition-transform duration-300 shadow-sm"
              :class="themeStore.isDark ? 'translate-x-7 left-0' : 'translate-x-1 left-0'"></div>
          </button>
        </div>

        <div
          class="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors">
          <div class="flex items-center gap-3">
      ...
      <div
        class="bg-white dark:bg-clinical-surface rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-8 overflow-hidden">
        <div
          class="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors">
