  ...
  <div class="min-h-screen bg-brand-bg-light dark:bg-brand-bg-dark pb-28 pt-4 lg:pt-8 text-brand-text dark:text-stone-100 font-sans transition-colors duration-300 overflow-hidden relative">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col gap-6 lg:gap-8 relative z-10">
      ...
      <div class="flex items-center justify-between">
        ...
        <button @click="router.push('/chat')" class="hidden sm:flex items-center gap-2 px-4 py-2 bg-brand-primary-light/50 dark:bg-brand-primary/10 hover:bg-brand-primary hover:text-white text-brand-primary rounded-xl text-xs font-bold transition-all border border-brand-primary/20 shadow-sm active:scale-95">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
        ...
        <div class="lg:col-span-4 bg-brand-surface-light dark:bg-brand-surface-dark border border-brand-surface-border dark:border-stone-800 rounded-[2.5rem] p-6 shadow-sm flex flex-col justify-between border-t-4 border-t-brand-primary">
          <div>
            ...
            <!-- Desktop View: Vertical stack / Mobile View: Swipable row -->
            <div
              ref="scrollContainer"
              class="flex overflow-x-auto lg:grid lg:grid-cols-1 gap-3 pb-4 lg:pb-0 mobile-snap-row hide-scrollbar"
              @scroll="handleMobileScroll"
            >
              <div
                v-for="axis in axes"
                :key="axis.letter"
                @click="openTypologyModal(axis)"
                class="bg-brand-bg-light dark:bg-stone-900/60 border-2 border-brand-surface-border dark:border-stone-800/80 rounded-2xl p-4 flex-shrink-0 w-[75vw] lg:w-full flex items-center justify-between hover:border-brand-primary/60 dark:hover:border-brand-primary/60 hover:shadow-md transition-all duration-300 cursor-pointer group active:scale-[0.99] snap-center"
              >
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h4 class="text-sm font-bold text-brand-text dark:text-white group-hover:text-brand-primary transition-colors">
                      {{ axis.name }}
                    </h4>
                    <p class="text-[10px] font-bold text-brand-text-muted uppercase tracking-wide mt-0.5">
                      vs. {{ axis.opposite }}
                    </p>
                  </div>
                </div>
          ...
          <div class="flex lg:hidden justify-center items-center gap-2.5 pt-2">
            <button
              ...
              @click="scrollToAxisCard(i)"
              class="h-2 rounded-full transition-all duration-300 cursor-pointer"
              :class="[activeMobileIndex === i ? 'w-5 bg-brand-primary' : 'w-2 bg-brand-surface-border hover:bg-brand-primary/50']"
            />
          </div>
        </div>

        <!-- COLUMN 2: Deep Nourishment Strategy & Concerns (4 Cols) -->
          ...
          <div class="bg-brand-surface-light dark:bg-brand-surface-dark border border-brand-surface-border dark:border-stone-800 rounded-[2.5rem] p-6 shadow-sm">
            ...
            <div class="flex flex-wrap gap-2">
              <span v-for="concern in profileData.commonConcerns" :key="concern"
                class="px-3 py-1.5 bg-brand-bg-light dark:bg-stone-900 rounded-xl text-[11px] font-bold text-brand-text dark:text-stone-200 border border-brand-surface-border dark:border-stone-800 shadow-2xs">
                {{ concern }}
              </span>
            </div>
          </div>
        ...
        <!-- COLUMN 3: Suggested Routine Blueprint (4 Cols) -->
